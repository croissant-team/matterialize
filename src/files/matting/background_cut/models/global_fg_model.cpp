...
Probability GlobalFgModel::global_probs(
  ...
  // everything might be background (we have no data to train the foreground
  // model)
  if (samples.empty()) {
    return Probability(
...
Mat GlobalFgModel::find_fg_samples(
  ...
  // max size is times 2 since we are taking samples from two images (current
  // and previous)
  ...
  for (int pixel{0}; pixel < curr_img.num_pixels(); pixel++) {
    ...
    // if pixel is in the intersection of F and the previous segmented
    // foreground, sample pixel from previous image to enforce temporal
    // coherence
